...
Код дашборда, скриптов синхронизации и деплоя добавляется по мере реализации (структура на усмотрение, но держать в репо всё необходимое для воспроизведения с `.env.example`).
