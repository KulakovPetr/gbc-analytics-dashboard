...
В `vercel.json` маршрут `GET /api/cron/sync` вызывается по расписанию. Сейчас: **каждые 10 минут** (`*/10 * * * *`).

- На **Hobby** Vercel разрешает cron **не чаще раза в сутки**; выражения вроде каждые 10 минут **заблокируют деплой** (ошибка про daily cron). Тогда верните, например, `0 9 * * *` или перейдите на **Pro** ([документация](https://vercel.com/docs/cron-jobs/usage-and-pricing)).
- На **Pro** можно до **раз в минуту** (`*/1 * * * *`) для краткого теста.
- Без смены плана: дергать синк вручную или внешним планировщиком (curl с заголовком `Authorization: Bearer <CRON_SECRET>` на URL продакшена).
